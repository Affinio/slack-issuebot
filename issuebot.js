...
//**** Setup
  ...
  if (process.env.organization) {
    defaultOrganization = process.env.organization;
  }
...
//**** Bot help
  ...
  controller.hears("issuebot (help|command)", messageEvents, function(bot, message){
    ...
    if (!defaultOrganization){
      ...
      bot.reply(message, "Your default organization is `"+defaultOrganization+"` you can use `reponame#123` directly");
    }
  });
//**** End bot help
...
//**** Bot configuration
  controller.hears("issuebot set default (organization|username) ([A-Za-z0-9-]+[^-]$)", messageEvents, function(bot, message){
    ...
    } // if we didn't set an organization on the command line
  });
...
//**** Repository parsing
  ...
  controller.hears("([A-Za-z0-9_.-]+)#([0-9]+)", messageEvents, function(bot,message) {
    ...
    if (repositoryMessage && repositoryMessage.length === 3){
      ...
      } // no command line organization set
      else {
        bot.reply(message, "https://github.com/"+defaultOrganization+"/"+reponame+"/issues/"+issueNum);
      }
    } // if we got proper repository information
    else { bot.reply(message, "Error parsing repository information"); }
  });
